...
Engine.Interface = function()
{
	...
	folderModel.add( parameters, 'modelScale').min(1).max(12).step(1).name('Model Scale').onChange(onChangeModelScale);
...
//    folderLOD.add( parameters, 'pow' ).name('Exponential').onChange(onSelectPow).listen();
    ...
    folderLOD.add( parameters, 'distanceLogScale' ).min(1).max(10).step(1).name('Scale Log').onChange(onChangeOptionOctree);
